...
int main() {
    ifstream fin("C:/Users/rares/Desktop/Sem2/Algoritmica grafelor/Probprac1/Probleme_pt_10/v3_orase_bidirectional_euclidian/input.txt");
    ...
    for (int i = 0; i < m; i++) {  // citim fiecare drum
        ...
        adj[x].push_back({y, cost});  // drum de la x la y
        adj[y].push_back({x, cost});  // drum de la y la x (bidirectional)
    }
    fin.close();
