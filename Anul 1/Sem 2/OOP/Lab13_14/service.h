...
/* Clasa Service contine logica aplicatiei: validare, filtrare, sortare
   Iteratia 2: lucreaza cu Repository& (polimorfism) si suporta undo
   Iteratia 4: implementeaza pattern-ul Observable (Subject din Observer Pattern)
   Tine o lista de Observer* (NU detine memoria - doar pointeri raw)
   Cand cosul (lista de spalare) se modifica, notifica toti observatorii */
class Service {
private:
    Repository& repo;                                         // referinta la repo
    std::vector<std::unique_ptr<ActiuneUndo>> actiuniUndo;    // stiva de actiuni reversibile

    /* Lista de Observer* - ferestrele care vor sa fie notificate
       NU detinem memoria observerilor (raw pointers, nu unique_ptr) -
       ferestrele Qt isi gestioneaza singure ciclul de viata */
...
public:
    /* Initializeaza service-ul cu o referinta la repository abstract
       Repository - foloseste vector intern + persistenta in fisier
    ...
    /* ===== OBSERVER PATTERN (cerinta iteratia 4) ===== */

    /* Inregistreaza un observer care va fi notificat la modificarea cosului
       Apelat de ferestrele CosCrud si CosForme in constructorul lor
       @param o: pointer la observer ; @return: - */
    void addObserver(Observer* o);

    /* Sterge un observer din lista (la inchiderea unei ferestre)
       Apelat in destructorul ferestrelor pentru a preveni notificari catre
       obiecte deja distruse (dangling pointers)
       @param o: pointer la observer ; @return: - */
    void removeObserver(Observer* o);

    /* Notifica toti observatorii ca s-a schimbat cosul
       Apelata automat dupa fiecare operatie care modifica lista de spalare
       (adauga, sterge, goleste, genereaza)
       @param: - ; @return: - */
    void notifyAll();
};
