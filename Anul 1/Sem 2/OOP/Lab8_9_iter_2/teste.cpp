...
/* Testeaza RepoDictionar - operatii de baza
   Folosim probabilitate 0.0 ca sa nu arunce erori aleatoare
   @param: - ; @return: - */
void sub_test_repo_dictionar() {
    RepoDictionar repo{ 0.0 };  // probabilitate 0 = nu esueaza niciodata
    ...
    // Adaugare duplicat => MasinaDuplicataException
    bool caught = false;
    try { repo.adauga(Masina{ "CJ 01 ABC", "BMW", "X5", "SUV" }); }
    catch (const MasinaDuplicataException&) { caught = true; }
    assert(caught);
    ...
    catch (const MasinaInexistentaException&) { caught = true; }
    assert(caught);

    // Modificare
    repo.modifica("CJ 01 ABC", "Audi", "A4", "sedan");
    assert(repo.cauta("CJ 01 ABC").getProducator() == "Audi");

    // Modificare inexistenta
    caught = false;
    try { repo.modifica("INEXISTENT", "X", "Y", "Z"); }
    catch (const MasinaInexistentaException&) { caught = true; }
    assert(caught);
    ...
    caught = false;
    try { repo.setStatusInchiriere("INEXISTENT", true); }
    catch (const MasinaInexistentaException&) { caught = true; }
    assert(caught);
...
/* Testeaza lista de spalare in RepoDictionar
   @param: - ; @return: - */
void sub_test_repo_dictionar_spalare() {
    RepoDictionar repo{ 0.0 };
    Masina m1{ "CJ 01 ABC", "Dacia", "Logan", "berlina" };

    assert(repo.lungimeListaSpalate() == 0);
    assert(!repo.existaInListaSpalate("CJ 01 ABC"));

    repo.adaugaInListaSpalate(m1);
    assert(repo.lungimeListaSpalate() == 1);
    assert(repo.existaInListaSpalate("CJ 01 ABC"));
    assert(repo.getListaSpalate().size() == 1);

    repo.golesteListaSpalate();
    assert(repo.lungimeListaSpalate() == 0);
}

/* Testeaza ca RepoDictionar arunca erori cu probabilitate 1.0 (intotdeauna)
   @param: - ; @return: - */
void sub_test_repo_dictionar_probabilitate() {
    // Probabilitate invalida => exceptie la constructor
    bool caught = false;
    try { RepoDictionar repoBad{ -0.5 }; }
    catch (const ValidareException&) { caught = true; }
    assert(caught);

    caught = false;
    try { RepoDictionar repoBad{ 1.5 }; }
    catch (const ValidareException&) { caught = true; }
    assert(caught);

    // Probabilitate 1.0 = intotdeauna esueaza
    RepoDictionar repo{ 1.0 };
    caught = false;
    try { repo.adauga(Masina{ "CJ 01 ABC", "Dacia", "Logan", "berlina" }); }
    catch (const ValidareException&) { caught = true; }
    assert(caught);
...
/* Testeaza polimorfismul - Service merge la fel cu Repository sau RepoDictionar
   @param: - ; @return: - */
void sub_test_polimorfism_repo() {
    // Cu Repository (vector)
    Repository repoVector;
    Service srv1{ repoVector };
    srv1.adaugaMasina("CJ 01 ABC", "Dacia", "Logan", "berlina");
    assert(srv1.getToateMasinile().size() == 1);

    // Cu RepoDictionar (map) - aceeasi interfata
    RepoDictionar repoMap{ 0.0 };
    Service srv2{ repoMap };
    srv2.adaugaMasina("CJ 01 ABC", "Dacia", "Logan", "berlina");
    assert(srv2.getToateMasinile().size() == 1);
}
...
/* Testeaza Repository::getAllRef care returneaza referinta constanta la vector
   (varianta optimizata fara copiere - pentru cazuri unde Service stie ca lucreaza
   cu Repository concret, nu cu RepoAbstract)
   @param: - ; @return: - */
void sub_test_repo_get_all_ref() {
    Repository repo;

    assert(repo.getAllRef().empty());
    assert(repo.getAllRef().size() == 0);

    repo.adauga(Masina{ "CJ 01 ABC", "Dacia", "Logan", "berlina" });
    repo.adauga(Masina{ "CJ 02 XYZ", "BMW", "X5", "SUV" });

    const auto& ref = repo.getAllRef();
    assert(ref.size() == 2);
    ...
    assert(repo.getAllRef().size() == 3);
}

/* Testeaza RepoDictionar::forEachMasina - itereaza fara copii prin callback
   @param: - ; @return: - */
void sub_test_repo_dictionar_for_each() {
    RepoDictionar repo{ 0.0 };

    int contor = 0;
    repo.forEachMasina([&contor](const Masina&) { ++contor; });
    assert(contor == 0);
    ...
    std::vector<std::string> nrInmatriculari;
    repo.forEachMasina([&nrInmatriculari](const Masina& m) {
        nrInmatriculari.push_back(m.getNrInmatriculare());
        });
    assert(nrInmatriculari.size() == 3);
    assert(nrInmatriculari[0] == "CJ 01 ABC");
    assert(nrInmatriculari[1] == "CJ 02 XYZ");
    assert(nrInmatriculari[2] == "IS 03 AAA");
}

/* Functia principala de test pentru iteratia 2
   @param: - ; @return: - */
void testIteratia2() {
    sub_test_repo_dictionar();
    sub_test_repo_dictionar_spalare();
    sub_test_repo_dictionar_probabilitate();
    sub_test_polimorfism_repo();
    ...
    sub_test_repo_dictionar_for_each();
}
