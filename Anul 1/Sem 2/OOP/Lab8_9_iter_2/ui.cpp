...
/* Anuleaza ultima actiune (cerinta iteratia 2)
   Apeleaza service.undo() care foloseste polimorfism pe ActiuneUndo
   @param: - ; @return: - */
void Ui::undoActiune() {
    try {
        service.undo();
        std::cout << "Ultima actiune a fost anulata cu succes!\n";
    }
    catch (const MasinaException& e) {
        std::cout << "Eroare: " << e.what() << "\n";
    }
...
void Ui::run() {
    while (true) {
        ...
        std::cout << "12. Undo (anuleaza ultima actiune)\n";
        std::cout << "0. Iesire\n";
        std::cout << "Comanda: ";
